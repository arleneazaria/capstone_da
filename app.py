...
def index():
    ...
    playstore_group = playstore.groupby(['Category','App']).agg({'Reviews':'sum', 'Rating':'mean'}).sort_values(by='Reviews', ascending=False).reset_index().head(10)
    ...
    stats = {
        ...
        'rev_table' : playstore_group.to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    type_order = df2.groupby('Type').agg({'Type' : 'count'}).rename({'Type':'Total'}, axis=1).sort_values(by='Type', ascending=False).reset_index().head()
    X = type_order['Type'].values.tolist()
    Y = type_order['Total'].values.tolist()
    fig = plt.figure(figsize=(5,5))
    fig.add_subplot()
    ...
    return render_template('index_capstone.html', stats=stats, result=result, result2=result2, result3=result3, result4=result4)
